...
export class PainterRepository implements IPainterRepository {
  ...
  public async createPainter(email: string, name: string, password: string): Promise<PainterDocument> {
    return this.painterModel.create({ email, name, password });
  }

  public async getPainterByEmail(email: string): Promise<PainterDocument> {
